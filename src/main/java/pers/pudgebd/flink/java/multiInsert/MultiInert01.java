...
//        streamEnv.setStreamTimeCharacteristic(TimeCharacteristic.EventTime);
        ...
        tableEnv.createTemporaryView("view2", ds);
        tableEnv.executeSql("insert into side_output_01 select sec_code, order_type from view1 where sec_code = 'p1'");

        Table so02 = tableEnv.sqlQuery("select sec_code, order_type from view2 where sec_code = 'p2'");
        DataStream<Row> so02Ds = tableEnv.toRetractStream(so02, Row.class)
                .map(tp2 -> tp2.f1)
                .returns(rowTypeInfo);
        tableEnv.fromDataStream(so02Ds).executeInsert("side_output_02");
